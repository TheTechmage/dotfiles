...
screen 1 do
  top    [ :views, :title, :center, :clock, :center, :spacer, :sublets ]
  bottom [ ]
end

# Example for a second screen:
screen 2 do
  top    [ :views, :title, :center, :fuzzytime, :center, :spacer, :tray, :membar, :freq ]
  bottom [ ]
  #top    [ :sublets, :spacer, :tray ]
...
style :all do
  ...
  border      "#303030", 0
  padding     0, 3, 3, 3
  #font "xft:Pragmata:pixelsize=9:antialias=true"
  font "-*-*-medium-*-*-*-14-*-*-*-*-*-*-*"
end
...
gravity :gimp_image,     [   0,   0, 100, 100 ]
gravity :gimp_toolbox,   [   0,   0,  10, 100 ]
gravity :gimp_dock,      [  90,   0,  10, 100 ]
gravity :fullscreen,	 [   0,   0, 100, 100 ]
gravity :vertCenter,	 [   0,   0, 100, 100 ], :horz
#
# == Grabs
#
...
# == System Keys
grab "XF86AudioRaiseVolume", "amixer -q set 'Speaker' 2+"
grab "XF86AudioLowerVolume", "amixer -q set 'Speaker' 2-"
grab "XF86AudioMute"       , "amixer -q set 'Speaker' toggle"
#XF86Display XF86Sleep
...
tag "browser" do
  match "uzbl|opera|firefox|navigator|chromium|chromium-sandmox"
  borderless true
  float true
  resize false
  gravity :fullscreen
...
view "com" do
  match       "irc"
  icon "#{iconpath}/bail.xbm"
  icon_only true
end
view "misc" do
  match "default"
  icon "#{iconpath}/exclamation1.xbm"
end
view "vnc" do
  match "vnc"
